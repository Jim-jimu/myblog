...
const DEFAULT_DEV_COMMAND =
	"if command -v pnpm >/dev/null 2>&1; then pnpm dev -- --host 127.0.0.1; else npm run dev -- --host 127.0.0.1; fi";
...
class TypefolioPreviewController implements vscode.Disposable {
	...
	constructor() {
		this.disposables.push(
			vscode.commands.registerCommand("typefolioPreview.openPreview", () => this.openPreview()),
			vscode.commands.registerCommand("typefolioPreview.refreshPreview", () =>
				this.refreshPreview(),
			),
			vscode.commands.registerCommand("typefolioPreview.startDevServer", () =>
				this.startDevServer(),
			),
			vscode.commands.registerCommand("typefolioPreview.openInBrowser", () => this.openInBrowser()),
	...
	private async refreshPreview() {
		...
		if (activeTarget) this.currentTarget = activeTarget;
		...
		if (config.saveBeforeRefresh && document.isDirty) await document.save();
	...
	private async startDevServer() {
		const workspaceFolder =
			this.currentTarget?.workspaceFolder ?? vscode.workspace.workspaceFolders?.[0];
		if (!workspaceFolder) {
	...
	private async handleActiveEditorChange(editor: vscode.TextEditor | undefined) {
		...
		if (!target) return;
		if (this.currentTarget?.documentUri.toString() === target.documentUri.toString()) return;
		this.currentTarget = target;
		await this.renderCurrentTarget();
	}

	private async handleDocumentSave(document: vscode.TextDocument) {
		if (!this.panel || !this.currentTarget) return;
		if (document.uri.toString() !== this.currentTarget.documentUri.toString()) return;

		const config = getPreviewConfig(this.currentTarget.workspaceFolder);
		if (!config.previewOnSave) return;
		await this.renderCurrentTarget();
	}
	...
	private ensurePanel() {
		...
		this.panel.webview.onDidReceiveMessage((message: { command?: string }) => {
			if (message.command === "refresh") void this.refreshPreview();
			if (message.command === "start") void this.startDevServer();
			if (message.command === "openExternal") void this.openInBrowser();
		});
	}

	private async renderCurrentTarget() {
		if (!this.panel || !this.currentTarget) return;
	...
	private resolveTarget(document: vscode.TextDocument): PreviewTarget | undefined {
		if (document.uri.scheme !== "file") return undefined;
		const extension = path.extname(document.uri.fsPath).toLowerCase();
		if (extension !== ".md" && extension !== ".mdx") return undefined;
...
function getPreviewConfig(workspaceFolder: vscode.WorkspaceFolder): PreviewConfig {
...
async function checkRoute(route: string): Promise<ServerStatus> {
...
async function waitForRoute(route: string, timeoutMs: number) {
	const started = Date.now();
	while (Date.now() - started < timeoutMs) {
		const status = await requestStatusCode(route);
		if (status && status >= 200 && status < 500) return;
		await delay(750);
	}
}

function requestStatusCode(route: string): Promise<number | undefined> {
...
function getFrameSource(route: string) {
