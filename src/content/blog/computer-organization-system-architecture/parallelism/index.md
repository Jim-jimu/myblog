...
- **原理解释：** 人类敲击键盘或移动鼠标的速度，在动辄以 GHz 计时的 CPU 看来是极其缓慢的。如果我们用轮询去检查键盘，CPU 可能会空转几亿次才等来一次按键。
...
虽然 CPU 每次处理中断都需要保存和恢复现场（这叫 Overhead，开销），但因为低速设备触发中断的频率非常低，所以把时间拉长来看，这种开销几乎可以忽略不计。
